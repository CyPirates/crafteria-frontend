  ...
  return (
    <CarouselContainer>
      <CarouselWrapper
        ...
        transition={isTransitioning ? `transform ${transitionTime}s ease-in-out` : 'none'}
      >
        {extendedImages.map((image, index) => (
          <CarouselSlide key={index} style={{ backgroundImage: `url(${image})` }} />
        ))}
      </CarouselWrapper>
      <PrevButton onClick={prevSlide}><IoIosArrowBack size={20}/></PrevButton>
      <NextButton onClick={nextSlide}><IoIosArrowForward size={20}/></NextButton>
    </CarouselContainer>
  );
};
